...
def train_model_search(train, valid, y_val, y_train):
    def objective(params):
        with mlflow.start_run():
            ...
            rfc = RandomForestClassifier(**params,random_state=seed)
            model_rfc = rfc.fit(train, y_train)
            
            #y_pred = decisionTree.predict(valid)

            y_scores = model_rfc.predict_proba(valid)
            auc = roc_auc_score(y_val, y_scores[:,1])
            mlflow.log_metric("auc", auc)
            mlflow.sklearn.log_model(model_rfc, "log_rfc_model")
        mlflow.end_run()

        return {'loss': -auc, 'status': STATUS_OK}

    search_space = {
        'max_depth': scope.int(hp.choice('max_depth', [10,12,14,15,16,17,18,19,20,21,22])),
        'criterion': hp.choice('criterion', ['gini', 'entropy']),
        'min_samples_split': hp.choice('min_samples_split', [7,8,9,10,12,13,14,15,16,17,18]),
        'min_samples_leaf': hp.choice('min_samples_leaf', [6,7,8,9,10,12,15,18]),
        'n_estimators': hp.choice('n_estimators', [10,11,12,15,17,18,19,20,22,24,26,28,30]),
...
def train_best_model(X_train, X_val, y_train, y_val, dict_encoders, best_params):
    with mlflow.start_run():
        ...
        y_scores = model_rfc.predict_proba(X_val)
        auc = roc_auc_score(y_val, y_scores[:,1])
        mlflow.log_metric("auc", auc)
        mlflow.sklearn.log_model(model_rfc, "log_rfc_model")
...
def main():
    ...
    mlflow.set_experiment("random-forest-alberta-wildfire-final")
    # mlflow.sklearn.autolog()

    df = read_dataframe()
    X_train, X_val, _, y_train, y_val, _, label_encoders = df_split(df)
